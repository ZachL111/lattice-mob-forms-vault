...
`lattice-mob-forms-vault` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for forms workflows, centered on simulation kernel, seeded input scenarios, and deterministic summary checks.
...
The score is deterministic. The current threshold is 169. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 5.
